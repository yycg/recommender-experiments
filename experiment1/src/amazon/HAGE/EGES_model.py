...
class EGES_Model:
    def __init__(self, num_nodes, num_feat, feature_lens, num_categories, num_category_feat, category_feature_lens,
        ...
        self.category_softmax_w = tf.Variable(tf.truncated_normal((num_nodes+num_categories, embedding_dim), stddev=0.1),
                                              name='category_softmax_w')
        self.category_softmax_b = tf.Variable(tf.zeros(num_nodes+num_categories), name='softmax_b')
        self.category_inputs = self.category_input_init()
        ...
        self.category_merge_emb = self.category_attention_merge()
        self.category_cost = self.make_category_skipgram_loss()
        self.category_train_op = tf.train.AdamOptimizer(lr).minimize(self.category_cost)
    ...
    def category_attention_merge(self):
        embed_list = []
        num_embed_list = []
        for i in range(self.num_category_feat):
            cat_embed = tf.nn.embedding_lookup(self.category_embedding[i], self.category_inputs[i])
            embed_list.append(cat_embed)
        stack_embed = tf.stack(embed_list, axis=-1)
        # attention merge
        alpha_embed = tf.nn.embedding_lookup(self.category_alpha_embedding, self.category_inputs[0])
        alpha_embed_expand = tf.expand_dims(alpha_embed, 1)
        alpha_i_sum = tf.reduce_sum(tf.exp(alpha_embed_expand), axis=-1)
        merge_emb = tf.reduce_sum(stack_embed * tf.exp(alpha_embed_expand), axis=-1) / alpha_i_sum
        return merge_emb

    def make_category_skipgram_loss(self):
        loss = tf.reduce_mean(tf.nn.sampled_softmax_loss(
            weights=self.category_softmax_w,
            biases=self.category_softmax_b,
            labels=self.category_inputs[-1],
            inputs=self.category_merge_emb,
            num_sampled=self.n_samped,
            num_classes=self.num_nodes+self.num_categories,
            num_true=1,
            sampled_values=tf.random.uniform_candidate_sampler(
                true_classes=tf.cast(self.inputs[-1], tf.int64),
                num_true=1,
                num_sampled=self.n_samped,
                unique=True,
                range_max=self.num_nodes+self.num_categories
            )
        ))
        return loss
