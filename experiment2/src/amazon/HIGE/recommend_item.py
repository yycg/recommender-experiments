...
def recommend_item(data_path, user_set, cand_set):
    ...
    wv = KeyedVectors.load_word2vec_format(os.path.join(data_path, "embedding", "node2vec11.embed"), binary=False)

    with open(os.path.join(data_path, "node2vec11.tsv"), "w") as recommend:
        for user in user_set:
            ...
            for cand in cand_set:
                if str(cand) in wv:
                    ...
                    score  = sum([dot(matutils.unitvec(vec), matutils.unitvec(wv[str(item)])) for item in user_items_train_map[user]])
