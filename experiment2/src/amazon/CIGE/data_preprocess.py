...
if __name__ == '__main__':
    ...
    with elapsed_timer("-- {0}s - %s" % ("get induced edge list",)):
        ...
        for index, row in sku_side_info.iterrows():
            item = row['sku_id']
            brand = row['brand']
            brand_items_map.setdefault(brand, [])
            brand_items_map[brand].append(item)
        
        # s_a: (a-v-v)
        with open("../../../data/amazon/s_a.csv", "w") as file:
            for brand, items in brand_items_map.items():
                permutations = itertools.permutations(items, 2)
                for permutation in permutations:
                    file.write(str(brand) + " " + " ".join([str(item) for item in permutation]) + "\n")
                for item in items:
                    file.write(str(brand) + " " + " ".join([str(item), str(item)]) + "\n")
