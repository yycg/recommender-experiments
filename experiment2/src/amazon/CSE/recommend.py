...
def recommend(data_path, user_set, cand_set):
    ...
    wv_user = KeyedVectors.load_word2vec_format(os.path.join(data_path, "embedding", "CSE_user.embed"), binary=False)
    wv_item = KeyedVectors.load_word2vec_format(os.path.join(data_path, "embedding", "CSE_item.embed"), binary=False)

    with open(os.path.join(data_path, "CSE.tsv"), "w") as recommend:
